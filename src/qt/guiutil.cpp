...
{
    // Convert zeal:// to zeal:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("zeal://"))
    {
        uri.replace(0, 12, "zeal:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "Zeal.lnk";
}
...
{
    return GetAutostartDir() / "zeal.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("Zeal-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  zeal-qt [" + tr("command-line options") + "]                     " + "\n";
...
void SetBlackThemeQSS(QApplication& app)
{
    app.setStyleSheet("QWidget        { background: rgb(41,44,48); }"
                      ...
                      "QToolButton:checked { color: rgb(255,255,255); border: none; border-left-color: rgb(253,178,32); border-left-style: solid; border-left-width: 6px; }"
